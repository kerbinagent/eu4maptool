...
thickBezier :: Float -> Float -> [(Float,Float)] -> [[(Float, Float)]]
thickBezier steplen thick input = zipWith connect (tail startEndPoints) (init startEndPoints) where
    (x0,y0) = head input
    (x1,y1) = input !! 1
    (x2,y2) = input !! 2
    (x3,y3) = last input
    getpoint = map (\t->((1-t)^three*x0+3*(1-t)^two*t*x1+3*(1-t)*t^two*x2+t^three*x3,(1-t)^three*y0+3*(1-t)^two*t*y1+3*(1-t)*t^two*y2+t^three*y3))
    getderivative = map (\t -> ( 3*(1-t)^two*(x1-x0)+6*(1-t)*t*(x2-x1)+3*t^two*(x3-x2) , 3*(1-t)^two*(y1-y0)+6*(1-t)*t*(y2-y1)+3*t^two*(y3-y2) ))
    compose (x,y) (dx,dy) = [ (x-thick*dy/sqrt (dx^two+dy^two), y+thick*dx/sqrt (dx^two+dy^two)), (x+thick*dy/sqrt (dx^two+dy^two), y-thick*dx/sqrt (dx^two+dy^two)) ]
    times = map ((*(arrOne !! (min 199 (steps-1)))) . fromIntegral) [0..min 199 (steps-1)] ++ [1]
    steps = dToIntDivide (sqrt((x1-x0)^two+(y1-y0)^two)+sqrt((x2-x1)^two+(y2-y1)^two)+sqrt((x3-x2)^two+(y3-y2)^two)) steplen
    connect = flip (++) . reverse
    startEndPoints = zipWith compose (getpoint times) (getderivative times)
